        ...
        public decimal CalculateColdPrice(Guid cargoID, DateTime start, DateTime end)
        {
            ...
            if (cargo.Billing.IsTiming)
            {
                ...
                foreach (var item in stockOuts)
                {
                    ...
                    decimal dailyFee = cargo.Billing.UnitPrice * Convert.ToDecimal(cargo.UnitVolume.Value) * item.StockOutDetails.Sum(r => r.Count) / 1000;
                ...
                foreach (var item in transferOuts)
                {
                    ...
                    decimal dailyFee = cargo.Billing.UnitPrice * Convert.ToDecimal(cargo.UnitVolume.Value) * item.TransferDetails.Sum(r => r.Count) / 1000;
                    totalFee -= (end.Subtract(item.ConfirmTime.Value).Days + 1) * dailyFee;
                }
            }
            else
            {
                totalFee = cargo.Billing.UnitPrice * Convert.ToDecimal(cargo.TotalVolume.Value);
            }
